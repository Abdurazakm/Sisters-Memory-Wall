...
const MessageItem = memo(({ message, isMine, editingId, setEditingId, setEditText, editText, onSave, onDelete, renderFile, setReplyTo }) => {
  ...
  return (
    <div className={`flex gap-2 w-full ${isMine ? "justify-end" : "justify-start"}`}>
      ...
      <div className={`relative max-w-[85%] sm:max-w-[70%] p-3 rounded-2xl shadow break-words ${isMine ? "bg-purple-500 text-white rounded-br-none" : "bg-white text-gray-900 rounded-bl-none"}`}>
          ...
          <div className="absolute top-1 right-1" ref={menuRef}>
            ...
            {openMenu && (
              <div className="absolute right-0 mt-1 w-32 bg-white text-gray-800 rounded-lg shadow-lg z-30 overflow-hidden border">
                <button onClick={() => { setReplyTo({ id: message.id, author: message.author, text: message.text, file_name: message.file_name }); setOpenMenu(false); }} className="w-full px-3 py-2 text-sm hover:bg-gray-100 flex items-center gap-2 border-b">
                  <FiCornerUpLeft size={14} /> Reply
                </button>
                {isMine && (
                  <>
                    <button onClick={() => { setEditingId(message.id); setEditText(message.text || ""); setOpenMenu(false); }} className="w-full px-3 py-2 text-sm hover:bg-gray-100 flex items-center gap-2 border-b">
                      <FiEdit2 size={14} /> Edit
                    </button>
                    <button onClick={() => { setOpenMenu(false); onDelete(message.id); }} className="w-full px-3 py-2 text-sm hover:bg-red-50 text-red-600 flex items-center gap-2">
        ...
        {isEditing ? (
          <div className="flex gap-1 items-center">
            <input value={editText} onChange={(e) => setEditText(e.target.value)} className="flex-1 px-2 py-1 rounded border text-gray-900 text-sm" autoFocus />
            <button onClick={() => onSave(message.id)} className="text-green-400"><FiCheck size={18} /></button>
            <button onClick={() => setEditingId(null)} className="text-gray-300"><FiX size={18} /></button>
          </div>
        ) : (
          <>
      ...
      {isMine && <div className="w-8 h-8 rounded-full bg-purple-600 text-white flex items-center justify-center font-bold shrink-0">{letter}</div>}
    </div>
  );
});
...
export default function MessageBoard() {
  ...
  const renderFileContent = useCallback((m) => {
    ...
    if (type === "image") return <img src={url} alt="upload" className="mt-2 rounded border max-h-60 object-cover cursor-pointer" onClick={() => window.open(url)} />;
  ...
  return (
    <section className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      <header className="p-3 bg-white shadow-sm flex items-center z-20">
        <button onClick={() => navigate("/")} className="flex items-center gap-2 text-purple-600 font-medium"><FiArrowLeft /> Back</button>
      </header>
      ...
      <footer className="bg-white border-t relative" ref={emojiPickerRef}>
        {replyTo && (
          <div className="absolute bottom-full left-0 w-full bg-purple-50 border-l-4 border-purple-500 px-4 py-2 text-sm flex justify-between items-center shadow-inner">
            ...
            <button onClick={() => setReplyTo(null)} className="text-gray-400 hover:text-gray-600"><FiX size={18} /></button>
          </div>
        )}
        ...
        <div className="p-3 flex items-center gap-2">
          ...
          <input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} placeholder="Type a message..." className="flex-1 bg-gray-100 border-none rounded-full px-4 py-2 text-sm focus:ring-2 focus:ring-purple-500" />
          <button onClick={sendMessage} disabled={!text.trim() && !replyTo} className={`p-2.5 rounded-full ${(text.trim() || replyTo) ? "bg-purple-600 text-white" : "bg-gray-200 text-gray-400"}`}><FiSend size={20} /></button>
        </div>
