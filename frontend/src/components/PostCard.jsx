...
export default function PostCard({ post, isMine, onRefresh, onOpenMedia }) {
  ...
  // Supabase returns the relationship as 'post_files' based on our schema
  const files = post.post_files || post.files || [];

  return (
    <div className="bg-white rounded-2xl shadow-sm border overflow-hidden transition-all hover:shadow-md mb-6">
      <div className="p-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 font-medium">
                {post.time ? new Date(post.time).toLocaleString() : "Just now"}
              </p>
            </div>
          </div>
              ...
              {showMenu && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-32 bg-white border rounded-xl shadow-lg z-20 overflow-hidden">
                    <button onClick={() => { setIsEditing(true); setShowMenu(false); }} className="w-full text-left px-4 py-3 text-sm hover:bg-purple-50 flex items-center gap-2">
                      <FiEdit2 size={14} /> Edit
                    </button>
                    <button onClick={() => { if(window.confirm("Delete post?")) deletePost(post.id).then(onRefresh); }} className="w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
        ...
        {isEditing ? (
          <div className="mb-4">
            ...
            <div className="flex gap-2 mt-2">
              <button onClick={async () => { await updatePost(post.id, editText); setIsEditing(false); onRefresh(); }} className="bg-purple-600 text-white px-4 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 shadow-sm">
                <FiCheck /> Save
              </button>
              <button onClick={() => setIsEditing(false)} className="bg-gray-100 px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-gray-200">
                Cancel
              </button>
            </div>
          </div>
        ...
        {files.length > 0 && (
          <div className={`grid gap-1 rounded-2xl overflow-hidden border mb-4 shadow-sm ${files.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
            {files.map((file, idx) => {
              // FIX: Determine if URL is local or remote (Supabase)
              const fileUrl = file.file_url.startsWith('http') 
                ? file.file_url 
                : `${BACKEND_URL}${file.file_url}`;

              return (
                <div key={idx} className="relative aspect-square bg-gray-100 flex items-center justify-center overflow-hidden cursor-pointer group" onClick={() => onOpenMedia(fileUrl, getFileType(file.file_type))}>
                  {getFileType(file.file_type) === "image" ? (
                    <img src={fileUrl} className="w-full h-full object-cover transition-transform group-hover:scale-105" alt="post content" />
                  ) : (
